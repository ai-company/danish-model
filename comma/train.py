...
def train_step(the_model, x, y):
    ...
    gradients, _ = tf.clip_by_global_norm(
        gradients, clip_norm=CLIPPING_THRESHOLD)
...
if __name__ == '__main__':
    ...
    optimizer = tf.keras.optimizers.Adagrad(
        learning_rate=learning_rate, initial_accumulator_value=1e-6)
    ...
    print(
        f'Looking for model in: {os.path.join(data.DATA_PATH, model_file_name)}')
    ...
    if path.exists(os.path.join(data.DATA_PATH, model_file_name)):
        ...
        net, stuff = model.load(os.path.join(
            data.DATA_PATH, model_file_name), x)
    ...
    for epoch in tqdm.tqdm(range(starting_epoch, MAX_EPOCHS), desc='Epochs'):
        ...
        for X, Y in tqdm.tqdm(get_minibatch(data.TRAIN_FILE, MINIBATCH_SIZE, shuffle=True), desc='Training'):
            ...
            if iteration % 100 == 0:
                ppl = np.exp(total_neg_log_likelihood /
                             total_num_output_samples)
                ...
                print(
                    f'At iteration {iteration}, processed sentences in epoch: {iteration * MINIBATCH_SIZE}')
                sys.stdout.write(f'PPL: {ppl}; Speed:{speed} sps\n')
                sys.stdout.flush()

            if iteration % 500 == 0:
                print(f'Saving for good measure')
                model.save(net, model_file_name, learning_rate=learning_rate, validation_ppl_history=validation_ppl_history,
                           best_validation_ppl=best_ppl, epoch=epoch, random_state=rng.get_state())
